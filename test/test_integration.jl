...
@testset "integrate" begin
    @testset "Basic Integration" begin
        ...
        @test length(solution.t) == length(0.0:dt:1.0)
        @test size(solution.u, 1) == length(solution.t)
        @test size(solution.u, 2) == 3
        @test solution.t[1] == 0.0
        @test solution.t[end] ≈ 1.0
        @test solution.u[1, :] ≈ u0
    end
    ...
    @testset "System Integration" begin
        ...
        @test solution.t[1] == system.tspan[1]
        @test solution.t[end] ≈ system.tspan[2]
        @test solution.u[1, :] ≈ system.u0
    end
    ...
    @testset "Different Time Spans" begin
        ...
        @test length(sol_short.t) ≈ 11 atol=1  # 0.1/0.01 + 1
        
        # Longer integration
        sol_long = integrate(params, u0, (0.0, 5.0), dt)
        @test length(sol_long.t) ≈ 501 atol=1  # 5.0/0.01 + 1
        
        # Both should start from same initial condition
        @test sol_short.u[1, :] ≈ sol_long.u[1, :]
    end
    ...
    @testset "Different Step Sizes" begin
        ...
        @test length(sol_coarse.t) < length(sol_fine.t)
        @test sol_coarse.u[1, :] ≈ sol_fine.u[1, :]  # Same initial condition
    ...
    @testset "Chaotic Behavior" begin
        ...
        @test all(-50 .< solution.u[:, 1] .< 50)  # x component
        @test all(-50 .< solution.u[:, 2] .< 50)  # y component
        @test all(0 .< solution.u[:, 3] .< 50)    # z component (mostly positive)
        
        # Should be non-periodic (chaotic)
        final_third = solution.u[end÷3:end, :]
